      ...
      <div className="space-y-8">
        ...
        <div className="max-w-[75%] mx-auto flex justify-center py-8 flex-wrap gap-8 lg:gap-3">
          {["lol", "valorant", "tft"].map((d: any, index: number) => (
            <span
              key={index}
              className="flex-shrink-0 w-16 h-16 rounded-full bg-violet-600 flex items-center justify-center p-3"
            >
              <img
                key={index}
      ...
      <div className="mt-6">
        ...
        <div className="mt-4 grid grid-cols-2 lg:grid-cols-3 gap-4">
          {jobs?.map((d: any, index: number) => (
            <div
              className={`rounded-lg bg-violet-950 bg-opacity-25 px-4 py-12 flex flex-col items-center justify-between gap-6`}
              key={index}
            >
